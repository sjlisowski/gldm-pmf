...
/**
 * This class kicks off a Job that the PMF's Redline Artwork Documents to the "Locked" status.
 *
 * The USER_ACTION usage is included for testing only.
 */

@RecordActionInfo(label="Lock Redline Artwork Documents", object = "pmf__c",
  usages={Usage.LIFECYCLE_ENTRY_ACTION, Usage.USER_ACTION})
public class LockRedlineArtworks implements RecordAction {

  public void execute(RecordActionContext recordActionContext) {

    JobService jobService = ServiceLocator.locate(JobService.class);
    JobParameters jobParameters = jobService.newJobParameters("pmf_lock_redline_artwork_documents__c");
